...
class EloquentUserRepository implements UserRepositoryContract
{
     ...
     * @return mixed
     */
    public function getUsersPaginated($per_page, $status = 1, $search, $order_by = 'id', $sort = 'asc') //get all users data + filter data
    {

        if($search == null) {

            return User::where('status', $status) //if search input is empty -> default retrive all user data
            ...
            return User::where('status', $status)
                ...
                ->OrWhere('board_number', 'like', $search)
