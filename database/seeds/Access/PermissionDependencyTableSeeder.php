...
class PermissionDependencyTableSeeder extends Seeder
{
    public function run()
    {
        ...
        $permission1Id = DB::table(config('access.permissions_table'))->where('name', 'view-backend')->first()->id;
        $permission2Id = DB::table(config('access.permissions_table'))->where('name', 'view-access-management')->first()->id;
         ...
         */
        $remainingPermissionsIds = DB::table(config('access.permissions_table'))->where('id', '>', 2)->pluck('id');
